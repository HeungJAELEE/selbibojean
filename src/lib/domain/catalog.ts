...
const groupSeeds: Record<number, GroupSeed[]> = {
  1: [
    ...
    ["센서·신호변환", ["센서", "변환기", "브리지", "열전대", "광전", "엔코더", "인코더", "퍼텐쇼미터", "싱크로"]],
  ...
  2: [
    ["용접 기초", ["용접", "용융", "모재", "용가재"]],
    ["아크용접", ["아크", "피복", "전극", "용접봉"]],
    ["가스·특수용접", ["가스용접", "산소", "아세틸렌", "TIG", "MIG", "서브머지드"]],
  ...
  4: [
    ...
    ["회전체 진동진단", ["회전체", "불평형", "불정렬", "축정렬", "진동진단", "베어링결함", "미끄럼·구름베어링", "기어 피팅", "기어 스폴링", "기어 리징", "가속도센서"]],
    ["소음·음향", ["소음", "음압", "데시벨", "음향", "주파수분석", "마스킹", "흡음", "음속", "잔향음장"]],
    ...
    ["TPM·자주보전·예방보전", ["TPM", "자주보전", "계획보전", "종합효율", "PM분석", "설비 로스", "6대 로스", "9대 로스", "만성로스", "불량·수정로스", "속도저하로스", "치명결점", "히스토그램", "QC 도구", "파레토도", "특성요인도", "가동률", "성능가동률", "품질불량"]],
...
export function mapConceptGroup(subjectCode: number, ...texts: Array<string | null | undefined>) {
  ...
  for (const candidate of candidates) {
    const score = candidate.keywords.reduce(
      (total, keyword) => total + (haystack.includes(keyword.toLowerCase()) ? keyword.length : 0),
      0,
    );
    if (score > bestScore) {
      best = candidate;
