...
const GROUP_RULES: Record<string, GroupRuleSet> = {
  ...
  "s2-g03": group("저항·가스·특수용접 종합", [
    rule("resistance", "저항용접", /저항용접|점용접|심용접|프로젝션용접|맞대기용접/),
    rule("gas", "가스용접", /가스용접|산소.*아세틸렌|중성불꽃|산화불꽃|탄화불꽃/),
    rule("thermit", "테르밋 용접", /테르밋/),
    rule("plasma", "플라즈마 아크용접", /플라즈마/),
    rule("beam", "전자빔·레이저용접", /전자빔|레이저/),
  ]),
  ...
  "s2-g05": group("작업안전 종합", [
    rule("electric", "감전·전기설비", /감전|전격|전기|접지|귀환|케이블|홀더/),
    rule("ppe", "보호구·광선·열·비산", /보호구|차광|광선|보호복|장갑|안전화|비산/),
    rule("fire", "화기작업·화재·폭발", /화기|화재|폭발|불티|감시자|소화/),
    rule("gas", "가스용기·조정기·호스", /가스|용기|조정기|호스|역화|역류/),
    rule("ventilation", "용접흄·환기·질식", /흄|환기|질식|밀폐|산소결핍/),
    rule("management", "장비점검·작업관리", /점검|정비|작업관리|연삭|고소|로봇/),
