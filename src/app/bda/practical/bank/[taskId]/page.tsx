...
import { ArrowLeft, ArrowRight, Code2, Database, ShieldCheck } from "lucide-react";
import { notFound } from "next/navigation";
import { getBdaCodeLabsForTask } from "@/data/source/bda-practical-content";
import {
  getBdaQbank,
...
export default async function BdaPracticalTaskPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  const codeLabs = getBdaCodeLabsForTask(
    task.practicalType,
    task.conceptIds,
  );
        ...
        <div className="grid gap-7 p-6 sm:p-9 lg:grid-cols-[1.2fr_.8fr]">
          <div>
            ...
            <section className="mt-7">
              <p className="eyebrow">Linked code labs</p>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {codeLabs.map((lab) => (
                  <Link
                    ...
                    className="group flex items-start justify-between gap-3 rounded-2xl border border-slate-200 p-4 hover:border-teal-300 hover:bg-teal-50"
                  >
                    <span>
                      <span className="flex items-center gap-2 text-xs font-black text-teal-800">
                        <Code2 size={14} /> {lab.category} ·{" "}
                        {lab.validation.status}
                      </span>
                      <strong className="mt-2 block leading-6 text-[#142f4b]">
                        {lab.title}
                      </strong>
                    </span>
                    <ArrowRight
